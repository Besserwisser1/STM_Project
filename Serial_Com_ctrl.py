...
class SerialCtrl():
    ...
    def SerialDataStream(self, gui):
        ...
        try:
            First = True
            while True:
                self.values = self.ser.readline()
                if First:
                    First = False
                    self.values = b''
                if self.values != b'':
                    gui.data.ClearData()
                    gui.data.DecodeMsg(self.values)
                    gui.data.AdjustData()
                    gui.UpdateGraph(gui.data.YDisplay, gui.data.XDisplay)
            ...
            # if max_counter != 0:
            #     # gui.data.RowMsg = self.ser.readline()
            #     values = values.decode('utf8').replace(',', '.').replace(' ', '')[:-1]
            #     gui.data.DecodeMsg(values)
            #     gui.data.AdjustDataCheck()
            # # gui.data.StreamDataCheck()
            # # if gui.data.StreamData:
